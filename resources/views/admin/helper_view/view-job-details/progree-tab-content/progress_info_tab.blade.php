<div class="mt-3 ms-3 me-3" id="progress_info" style="display:none">
    <div class="ms-3 me-3">
        <div class="table-responsive table-card mb-1">
            @if ($count_inprogress_candidate > 0)
                <table class="table align-middle" id="customerTable">
                    ...
                    <tbody class="list form-check-all">
                        @foreach ($get_all_job_applicants as $source_candidate)
                            @if (in_array($source_candidate->applicant_status,get_inprogress_applicant_status_list()))
                                <tr>
                                    <th scope="row">
                                        <div class="form-check">
                                    ...
                                    <td style="text-align: center">
                                        ...
                                        @if (in_array($source_candidate->applicant_status,get_inprogress_applicant_not_appeared_status_list()))
                                            <a href="{{ route('view_applicant_update_progress', ['applikey' => $source_candidate->applicant_key]) }}"
                                        ...
                                        @endif
                                        @if (in_array($source_candidate->applicant_status,get_inprogress_applicant_appeared_status_list()))
                                            <a href="{{ route('update_hiring_pool_appeared', ['applikey' => $source_candidate->applicant_key]) }}"
                                                data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top"
                                                title="Change Status For {{ $source_candidate->candidate_name }}">
                                                <button class="btn btn-soft-secondary btn-sm " type="button"data-bs-toggle="tooltip" title="Update">
                                                    <i class="ri-edit-fill align-middle me-1"></i><span class="fs-10">Update</span>
